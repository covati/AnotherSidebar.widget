...
options =
  ...
  city          : "Durham"          # default city in case location detection fails
  region        : "NC"              # default region in case location detection fails
...
style: """
  ...
  *, *:before, *:after
  ...
  @font-face
    font-family Weather
    src url(AnotherSidebar.widget/WeatherNow.widget/icons.svg) format('svg')
...
command: "#{process.argv[0]} AnotherSidebar.widget/WeatherNow.widget/get-weather \
                            \"#{options.city}\" \
                            \"#{options.region}\" \
                            #{options.units} \
...
render: -> """
  <div class='wrapper #{@appearance.iconSet} sidebar'>
    <div class='icon'></div>
    <div class='text'>
